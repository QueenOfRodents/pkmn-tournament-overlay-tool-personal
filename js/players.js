...
function findPlayerByUuid(uuid) {
  return PLAYER_LIST.find((player) => {
    return player.uuid && player.uuid === uuid;
  });
}
...
function addPlayer(existingData) {
  const playerData =
    existingData && existingData.uuid
      ? existingData
      : {
          uuid: uuidv4(),
        };
  ...
  for (let monIndex = 1; monIndex <= 6; monIndex++) {
    const monInput = row.querySelector(
      `#player_${playerData.uuid}_mon_${monIndex}`,
    );
    const validateMon = () => {
      validate(monInput);
    };
    ...
    const itemInput = row.querySelector(
      `#player_${playerData.uuid}_mon_${monIndex}_item`,
    );
    const validateItem = () => {
      validate(itemInput);
    };
    ...
    const teraInput = row.querySelector(
      `#player_${playerData.uuid}_mon_${monIndex}_tera`,
    );
    const validateTera = () => {
      validate(teraInput);
    };
  ...
  deleteButton.addEventListener("click", () => {
    ...
    row.remove();
    const entry = PLAYER_LIST.findIndex(
      (player) => player.uuid === playerData.uuid,
    );
  ...
  const importButton = row.querySelector(
    `#player_${playerData.uuid}_import_paste`,
  );
  importButton.addEventListener("click", () => {
    ...
    if (text && text.length > 0) {
      ...
      for (let i = 0; i < mons.length; i++) {
        const monInput = row.querySelector(
          `#player_${playerData.uuid}_mon_${i + 1}`,
        );
        monInput.value = mons[i].species ?? "";
        const event = new Event("change");
        const itemInput = row.querySelector(
          `#player_${playerData.uuid}_mon_${i + 1}_item`,
        );
        itemInput.value = mons[i].item ?? "";
        const teraInput = row.querySelector(
          `#player_${playerData.uuid}_mon_${i + 1}_tera`,
        );
...
document.getElementById("player_add").addEventListener("click", () => {
  addPlayer();
});
...
async function importPlayersFromTOM(file, args) {
  ...
  const filtered = players.filter(
    (player) => !currentPlayerNames.includes(player.name),
  );
...
async function importStandingsFromTOM(file) {
  ...
  for (let i = 0; i < standingModules.length; i++) {
    if (standings.allStandings.length > i) {
      ...
      const selectedIndex = [...standingModules[i].options].findIndex(
        (opt) => opt.value === uuid,
      );
      standingModules[i].options[
  ...
  for (let player of standings.allStandings) {
    for (let opt of [...playerOptions].filter(
      (o) => o.value === findPlayerByName(player.name)?.uuid,
    )) {
      opt.setAttribute(
        "record",
        `(${player.record.wins}/${player.record.ties}/${player.record.losses})`,
      );
    }
  }
}
  ...
  for (let i = 0; i < pairingModules.length; i++) {
    if (pairings.pairings.length > i) {
      const playerSelectors =
        pairingModules[i].querySelectorAll(".playerSelect");
      const setPlayer = (playerName, selectorIndex) => {
        const uuid = findPlayerByName(playerName)?.uuid;
        const selectedIndex = [
          ...playerSelectors[selectorIndex].options,
        ].findIndex((opt) => opt.value === uuid);
        playerSelectors[selectorIndex].options[
          selectedIndex > 0 ? selectedIndex : 0
        ].selected = true;
      };
