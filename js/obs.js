const OBS = {
  ...
  populateSourceOptionsFromScene(sceneName, target) {
    const browserSourceOptions = document.getElementById(
      `browserSourceOptions_${target}`,
    );
    ...
    const textSourceOptions = document.getElementById(
      `textSourceOptions_${target}`,
    );
    ...
    if (OBS.obs.identified) {
      try {
        ...
        const filtered = allItems
          .filter(
            (item) => item.inputKind && item.inputKind.includes(sourceType),
          )
          .map((item) => item.sourceName);
        return filtered;
      } catch (e) {
        console.warn(
          `Error fetching sources of type ${sourceType} from scene ${sceneName}: ${e}`,
        );
        return [];
      }
    }
  ...
  async getBrowserSourcesInScene(sceneName) {
    return await OBS.getSourcesOfTypeInScene(
      OBS.SOURCE_TYPE.BROWSER,
      sceneName,
    );
  },
