...
function createFromTemplates() {
  ...
  for (let j = 1; j <= 2; j++) {
    for (let i = 6; i > 0; i--) {
      const item = monModuleTemplate.content
        .cloneNode(true)
        .querySelector(".monModule");
      ...
      for (let element of elements) {
        ...
        if (element.getAttribute("for")) {
          element.setAttribute(
            "for",
            element.getAttribute("for").replace("_x", `_${uuid}`),
          );
        }
      }
      node.after(item);
...
function attachEventListeners() {
  ...
  for (let playerModule of playerModules) {
    ...
    jump?.addEventListener("click", () => {
      ...
      const selectedOption =
        playerSelector.options[playerSelector.options.selectedIndex];
    ...
    for (let monModule of monModules) {
      ...
      const setItem = () => {
        const item =
          monSelector.options[monSelector.selectedIndex].getAttribute("item");
      ...
      const updateIcon = () => {
        ...
        const teraType =
          monSelector.options[monSelector.selectedIndex].getAttribute("tera") ??
          undefined;
        ...
        const anyTeraToggled = [
          ...playerModule.querySelectorAll(".teraToggle"),
        ].find((t) => t.checked);
        ...
        if (itemOpt) {
          if (itemOpt.type === "Berry") {
            ...
            url.searchParams.set(
              "item",
              `item_icon_${itemOpt.getAttribute("key")}`,
            );
          }
      ...
      monSelector.addEventListener("change", () => {
        const elementList = [...monModules].map((item) =>
          item.querySelector(".monSelect"),
        );
    ...
    fillButton?.addEventListener("click", () => {
      for (let i = 0; i < monModules.length; i++) {
        ...
        if (
          monSelector.options.length > i &&
          !monSelector.options[i + 1].classList.contains("notRegistered")
        ) {
  ...
  for (let nameModule of nameModules) {
    ...
    const updatePlayer = () => {
      ...
      if (sourceSelector) {
        const selectedOption =
          playerSelector.options[playerSelector.options.selectedIndex];
        const playerName =
          selectedOption && selectedOption.value !== selectedOption.innerText
        ...
        OBS.setTextSourceText(
          sourceSelector.value,
          `${prefix} ${playerName} ${suffix}`.trim(),
        );
      }
    };
    ...
    playerSelector.addEventListener("change", () => {
      ...
      const elementList = [
        ...playerSelector.closest(".section")?.querySelectorAll(".nameModule"),
      ].map((i) => i.querySelector(".playerSelect"));
      removeSelectedOptions(elementList);
      if (playerModule) {
  ...
  resetRoundButton.addEventListener("click", () => {
    const description = [...resetRoundButton.querySelectorAll("li")]
      .map((item) => `• ${item.innerText}`)
      .join("\n");
    if (
      window.confirm(
        `Do you really want to reset the round?\nThis action will do the following:\n${description}`,
      )
    ) {
      const playerSelectors = document
        .getElementById("battle")
        .querySelectorAll(".playerSelect");
      // Set both players to 'None'
      for (let playerSelector of playerSelectors) {
        resetSelector(playerSelector, true);
      }
      const scoreModules = document
        .getElementById("battle")
        .querySelectorAll(".scoreModule");
      for (let scoreModule of scoreModules) {
  ...
  resetMatchButton.addEventListener("click", () => {
    const description = [...resetMatchButton.querySelectorAll("li")]
      .map((item) => `• ${item.innerText}`)
      .join("\n");
    if (
      window.confirm(
  ...
  resetPairingsButton.addEventListener("click", () => {
    const description = [...resetPairingsButton.querySelectorAll("li")]
      .map((item) => `• ${item.innerText}`)
      .join("\n");
    if (
      window.confirm(
        `Do you really want to reset the pairings display?\nThis action will do the following:\n${description}`,
      )
    ) {
      // Effectively 'click' both reset buttons
      const pairingModules = pairingsList.querySelectorAll(".pairingsModule");
      for (let pairingModule of [...pairingModules]) {
        for (let playerSelect of [
          ...pairingModule.querySelectorAll(".playerSelect"),
        ]) {
  ...
  resetStandingsButton.addEventListener("click", () => {
    const description = [...resetStandingsButton.querySelectorAll("li")]
      .map((item) => `• ${item.innerText}`)
      .join("\n");
    if (
      window.confirm(
        `Do you really want to reset the standings display?\nThis action will do the following:\n${description}`,
      )
    ) {
      const standingsList = document.getElementById("standingsList");
      for (let playerSelect of [
        ...standingsList.querySelectorAll(".playerSelect"),
      ]) {
        resetSelector(playerSelect, true);
      }
    }
  ...
  const filterPlayerTable = () => {
    ...
    for (let row of rows) {
      ...
      if (
        playerFilter.value &&
        !name.value.toLowerCase().includes(playerFilter.value.toLowerCase())
      ) {
        row.hidden = true;
      } else {
        i++;
  ...
  document
    .getElementById("playerImport")
    .addEventListener("input", async (e) => {
      try {
        const playerImportStatus =
          document.getElementById("playerImportStatus");
        const newPlayers = await importPlayersFromTOM(e.target.files[0], {
          abbreviateJuniors: document.getElementById("abbreviateJuniorsToggle")
            .checked,
          abbreviateSeniors: document.getElementById("abbreviateSeniorsToggle")
            .checked,
          abbreviateMasters: document.getElementById("abbreviateMastersToggle")
            .checked,
        });
        ...
        playerImportStatus.innerText =
          "Selected file is either malformed or not the ...roster.html file!";
      }
    });

  // Hook up Pairings/Pairings Import
  document
    .getElementById("pairingsImport")
    .addEventListener("click", async () => {
      try {
        const file = await selectFile("tomDirectory");
        window.clearInterval(pairingsInterval);
        document.getElementById("pairingsImportFile").innerText = abridgeWord(
          file.name,
        );
        ...
        try {
          ...
          status.innerText =
            "Selected file is either malformed or not the ...pairings.html file!";
        }
  ...
  document
    .getElementById("pairingsTrackingStop")
  ...
  const updatePairingsSingle = () => {
    ...
    const splitter =
      document.getElementById("pairingsSingleSplitter")?.value ?? "";
    const pairingsModules = [
      ...document
        .getElementById("pairingsList")
        .querySelectorAll(".pairingsModule"),
    ];
    const pairings = [];
    ...
    for (let i = 0; i < pairingsModules.length; i++) {
      if (!pairingsModules[i].hidden) {
        const playerSelectors = [
          ...pairingsModules[i].querySelectorAll(".playerSelect"),
        ];
        // P1
        const selectedOption1 =
          playerSelectors[0].options[playerSelectors[0].options.selectedIndex];
        const player1Name =
          selectedOption1 && selectedOption1.value !== selectedOption1.innerText
            ? selectedOption1.innerText
            : UNKNOWN_PLAYER;
        // P2
        const selectedOption2 =
          playerSelectors[1].options[playerSelectors[1].options.selectedIndex];
        const player2Name =
          selectedOption2 && selectedOption2.value !== selectedOption2.innerText
            ? selectedOption2.innerText
            : UNKNOWN_PLAYER;
        if (player1Name !== UNKNOWN_PLAYER && player2Name !== UNKNOWN_PLAYER) {
          pairings.push(
            `Table ${i + 1}: ${player1Name} vs. ${player2Name} ${splitter} `,
          );
        }
      }
    }
  ...
  const pairingsModules = document
    .getElementById("pairingsList")
    .querySelectorAll(".pairingsModule");
  for (let pairingsModule of pairingsModules) {
    const updatePairingsPlayers = () => {
      if (!pairingsModule.hidden) {
        ...
        const playerNames = [];
        const playerSelectors =
          pairingsModule.querySelectorAll(".playerSelect");
        for (let playerSelector of playerSelectors) {
          const selectedOption =
            playerSelector.options[playerSelector.options.selectedIndex];
          const playerName =
  ...
  document
    .getElementById("pairingsSingleSplitter")
  ...
  document
    .getElementById("standingsImport")
    .addEventListener("click", async () => {
      try {
        const file = await selectFile("tomDirectory");
        window.clearInterval(standingsInterval);
        document.getElementById("standingsImportFile").innerText = abridgeWord(
          file.name,
        );
        ...
        try {
          ...
          status.classList.add("disconnected");
          status.innerText =
            "Selected file is either malformed or not the ...standings.html file!";
        }
  ...
  document
    .getElementById("standingsTrackingStop")
    .addEventListener("click", () => {
  ...
  const updateStandingsSingle = () => {
    ...
    const splitter =
      document.getElementById("standingsSingleSplitter")?.value ?? "";
    const includeOrdinal = document.getElementById(
      "standingsSingleOrdinalToggle",
    ).checked;
    const standingsModules = [
      ...document
        .getElementById("standingsList")
        .querySelectorAll(".standingsModule"),
    ];
    const placements = [];
    for (let i = 0; i < standingsModules.length; i++) {
      if (!standingsModules[i].hidden) {
        const playerSelector =
          standingsModules[i].querySelector(".playerSelect");
        const placeSuffix = includeOrdinal
          ? applyOrdinalSuffix(i + 1) + " "
          : "";
        const selectedOption =
          playerSelector.options[playerSelector.options.selectedIndex];
        const playerName =
  ...
  const standingsPlayerSelectors = document
    .getElementById("standingsList")
    .querySelectorAll(".playerSelect");
  ...
  document
    .getElementById("standingsSingleSplitter")
    .addEventListener("change", () => {
      updateStandingsSingle();
    });

  document
    .getElementById("standingsSingleOrdinalToggle")
    .addEventListener("change", () => {
      updateStandingsSingle();
    });

  document
    .getElementById("standingsOrdinalToggle")
    .addEventListener("change", () => {
      for (let playerSelector of standingsPlayerSelectors) {
        const event = new Event("change");
  ...
  document.getElementById("standingsSlider").addEventListener("input", () => {
    const standingsModules = document
      .getElementById("standingsList")
      .querySelectorAll(".standingsModule");
  ...
  for (let sceneSelector of sceneSelectors) {
    sceneSelector.addEventListener("change", () => {
      OBS.populateSourceOptionsFromScene(
        sceneSelector.value,
        sceneSelector.getAttribute("target"),
      );
    });
  }
}
...
function loadSourceSettings() {
  ...
  if (settings.battleSources) {
    ...
    for (let i = 0; i < playerModules.length; i++) {
      ...
      const sources = settings.battleSources[i]
        ? settings.battleSources[i]
        : [];
      const sourceSelectors = [
        ...playerModule.querySelectorAll(".sourceSelect"),
      ];
  ...
  if (settings.standingsSources) {
    const standingsSourceSelectors = document
      .getElementById("standingsList")
      .querySelectorAll(".sourceSelect");
  ...
  if (settings.standingsSingleSource) {
    document.getElementById("standingsSingleSource").value =
      settings.standingsSingleSource;
  }
  ...
  if (settings.pairingsSources) {
    const pairingsSourceSelectors = document
      .getElementById("pairingsList")
      .querySelectorAll(".sourceSelect");
  ...
  if (settings.pairingsSingleSource) {
    document.getElementById("pairingsSingleSource").value =
      settings.pairingsSingleSource;
  }
...
function loadGeneralSettings() {
  ...
  const event = new Event("input");
  document.getElementById("abbreviateJuniorsToggle").checked =
    settings.abbreviateJuniors;
  document.getElementById("abbreviateSeniorsToggle").checked =
    settings.abbreviateSeniors;
  document.getElementById("abbreviateMastersToggle").checked =
    settings.abbreviateMasters;

  document.getElementById("pairingsSingleSplitter").value =
    settings.pairingsSingleSplitter ?? "/";
  document.getElementById("pairingsRecordToggle").checked =
    settings.pairingsIncludeRecord;
  ...
  document.getElementById("standingsOrdinalToggle").checked =
    settings.standingsIncludeOrdinal;
  document.getElementById("standingsRecordToggle").checked =
    settings.standingsIncludeRecord;
  document.getElementById("standingsSingleOrdinalToggle").checked =
    settings.standingsSingleIncludeOrdinal;
  document.getElementById("standingsSingleSplitter").value =
    settings.standingsSingleSplitter ?? "/";
  document.getElementById("standingsSlider").value =
    settings.standingsCount ?? 8;
  document.getElementById("standingsSlider").dispatchEvent(event);

  document.getElementById("battleRecordToggle").checked =
    settings.battleIncludeRecord;

  document.getElementById("monCountSlider").value =
    settings.monsPerTeamCount ?? 4;
...
function saveGeneralSettings() {
  ...
  const settings = {
    ...
    abbreviateMasters: document.getElementById("abbreviateMastersToggle")
      .checked,
    pairingsIncludeRecord: document.getElementById("pairingsRecordToggle")
      .checked,
    pairingsSingleSplitter: document.getElementById("pairingsSingleSplitter")
      .value,
    pairingsCount: document.getElementById("pairingsSlider").value,
    standingsIncludeOrdinal: document.getElementById("standingsOrdinalToggle")
      .checked,
    ...
    standingsSingleSplitter: document.getElementById("standingsSingleSplitter")
      .value,
...
window.onload = async () => {
  ...
  loadItemdex((item) => {
    ...
    opt.setAttribute(
      "key",
      item.name.toLowerCase().replaceAll(" ", "_").replaceAll("'", ""),
    );
    opt.type = item.type;
